...
#         tr_bd_ix = trace_obj[6]
    ...
    def solar_light(self,building_ids,proportion_consumption,proportion_green):
        for b_id in building_ids:
            for i in range(self.traces_end_index[self.b_id_hash[b_id]],self.traces_end_index[self.b_id_hash[b_id] + 1]):
                ...
                num_on = int(proportion_consumption*self.b_id_mapping[b_id])
                max_on_ix = self.traces_end_index[self.b_id_hash[b_id]] + num_on
                print(f"curr value of max green num is: {num_green}")
                print(f"current value of max on is {num_on}")
                if i < max_green_ix:
                    ...
                    if i < max_on_ix:
                        ...
                        self.strip.set_pixel(i, self.blank)
